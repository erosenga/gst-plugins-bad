...
_do_create_memory (GstGLContext * context, ContextThreadData * data)
{
  ...
  switch (GST_VIDEO_INFO_FORMAT (&cache->input_info)) {
      case GST_VIDEO_FORMAT_BGRA:
        ...
        memory = gst_apple_core_video_memory_new_wrapped (context, gpixbuf,
            CVOpenGLESTextureGetName (texture), gl_target,
            GST_VIDEO_GL_TEXTURE_TYPE_RGBA, &cache->input_info,
      ...
      case GST_VIDEO_FORMAT_NV12: {
        ...
        gl_target = gst_gl_texture_target_from_gl (CVOpenGLESTextureGetTarget (texture));
        memory = gst_apple_core_video_memory_new_wrapped (context, gpixbuf,
            CVOpenGLESTextureGetName (texture), gl_target, textype, &cache->input_info,
  ...
  data->memory = GST_MEMORY_CAST (memory);
...
gst_video_texture_cache_create_memory (GstVideoTextureCache * cache,
      ...
      guint plane)
{
  ContextThreadData data = {cache, gpixbuf, plane, NULL};
