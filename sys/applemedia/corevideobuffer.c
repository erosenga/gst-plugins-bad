...
static GstMemory *
gst_core_video_create_gl_memory (GstAppleCoreVideoPixelBuffer * gpixbuf,
    GstVideoInfo * info, guint plane, guint size, GstVideoTextureCache * cache)
{
#if HAVE_IOS
  return gst_video_texture_cache_create_memory (cache, gpixbuf, plane);
#else
  ...
  if (CVPixelBufferIsPlanar (pixel_buf)) {
    ...
    for (i = 0; i < n_planes; i++) {
      ...
      gst_buffer_append_memory (buf,
          gst_core_video_create_gl_memory (gpixbuf, info, i, size, cache));
    }
  } else {
    n_planes = 1;
    ...
    gst_buffer_append_memory (buf,
        gst_core_video_create_gl_memory (gpixbuf, info, 0, size, cache));
  }
